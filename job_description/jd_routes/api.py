...
def create_job(payload: JobCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Call 8000 without requiring a file
        response = requests.post("http://localhost:8000/admin/jd/sync", json=sync_payload, timeout=15)
        if response.status_code == 201:
            print(f"✅ Fast-forwarded JD {job.id} intelligence sync to 8000")
        else:
            print(f"⚠️ Failed to sync JD {job.id} to 8000. Status: {response.status_code}, Res: {response.text}")
    except Exception as e:
...
def update_job(job_id: str, payload: JobCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        response = requests.post("http://localhost:8000/admin/jd/sync", json=sync_payload, timeout=15)
        if response.status_code == 201:
            print(f"✅ Fast-forwarded JD {job.id} intelligence sync to 8000 (Update)")
        else:
            print(f"⚠️ Failed to sync JD {job.id} to 8000 (Update). Status: {response.status_code}, Res: {response.text}")
    except Exception as e:
        print(f"Error calling 8003 backend: {e}")
