...
export default function Step4Review({ personal, careerDetails, experience, job, onBack, onEdit, onSubmit }) {
  ...
  return (
    <div className="step-container">
      ...
      {/* Career Details */}
      <ReviewBlock title="Career Details" icon="💼" onEdit={onEdit} editStep={2}>
        <div className="review-grid">
          <ReviewRow label="Total Experience"    value={careerDetails.totalExperience ? `${careerDetails.totalExperience} yrs` : null} />
          <ReviewRow label="Relevant Experience" value={careerDetails.relevantExperience ? `${careerDetails.relevantExperience} yrs` : null} />
          <ReviewRow label="Notice Period"       value={careerDetails.noticePeriod} />
          <ReviewRow label="Current CTC"         value={careerDetails.currentCtc ? `₹${careerDetails.currentCtc} LPA` : null} />
          <ReviewRow label="Expected CTC"        value={careerDetails.expectedCtc ? `₹${careerDetails.expectedCtc} LPA` : null} />
          <ReviewRow label="Open to Relocate"    value={careerDetails.openToRelocate} />
          <ReviewRow label="LinkedIn"            value={careerDetails.linkedinProfile} />
        </div>
        {(careerDetails.locationPreference || []).length > 0 && (
          <div className="review-address">
            <span className="review-label">Location Preference</span>
            <span className="review-value">{(careerDetails.locationPreference || []).join(', ')}</span>
          </div>
        )}
      </ReviewBlock>

      {/* Work Experience */}
      <ReviewBlock title="Work Experience" icon="🏢" onEdit={onEdit} editStep={3}>
      ...
      {/* Education */}
      <ReviewBlock title="Education" icon="🎓" onEdit={onEdit} editStep={3}>
        {educations.length === 0 ? (
          <EmptyState msg="No education added." />
        ) : (
          educations.map((e, i) => (
            <div key={i} className="review-exp-item">
              <div className="review-exp-title">{e.degree} {e.discipline ? `· ${e.discipline}` : ''}</div>
              <div className="review-exp-company">{e.school}</div>
              <div className="review-exp-dates">
                {e.startMonth ? `${MONTH_NAMES[+e.startMonth]} ` : ''}{e.startYear}
                {' → '}
                {e.endMonth ? `${MONTH_NAMES[+e.endMonth]} ` : ''}{e.endYear}
              </div>
            </div>
          ))
        )}
      ...
      {/* Certifications */}
      <ReviewBlock title="Certifications" icon="🏅" onEdit={onEdit} editStep={3}>
          ...
          <div className="review-certs">
            {certifications.map((c, i) => (
              <div key={i} className="review-cert-tag">
                ...
                {c.fileName && <span className="cert-file-badge">📎 {c.fileName}</span>}
              </div>
            ))}
          </div>
        )}
      ...
      {/* Resume */}
      <ReviewBlock title="Resume" icon="📄" onEdit={onEdit} editStep={3}>
        {resumeName ? (
      ...
      <div className="wizard-nav">
        <button className="btn-back" onClick={onBack} id="btn-step4-back">
