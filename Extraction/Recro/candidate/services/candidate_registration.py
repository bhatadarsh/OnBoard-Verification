...
class CandidateService:
    ...
    def apply(jd_id: str, payload, candidate_id: str, db: Session):
        ...
        if existing_education:
            ...
            if existing_edu_data != new_edu_data:
                ...
                for edu in payload.education:
                    db.add(CandidateEducation(
                        candidate_id          = candidate_id,
                        school                = edu.highest_qualification,
                        degree                = edu.degree_name,
                        discipline            = edu.university,
                        start_month           = edu.start_month,
                        end_month             = edu.end_month,
                        start_year            = edu.start_year,
                        end_year              = edu.end_year,
        ...
        if payload.experiences:
            ...
            if existing_exp:
                ...
                for exp in payload.experiences:
                    # Parse start/end dates (YYYY-MM format) into separate month/year
                    s_parts = (exp.start_date or '').split('-')
                    e_parts = (exp.end_date or '').split('-')
                    db.add(CandidateExperience(
                        ...
                        open_to_relocate        = exp.open_to_relocate,
                        resume_path             = payload.resume_path,
                        is_current_designation  = exp.is_current or False,
                        start_month             = int(s_parts[1]) if len(s_parts) == 2 else None,
                        start_year              = int(s_parts[0]) if len(s_parts) >= 1 and s_parts[0] else None,
                        end_month               = int(e_parts[1]) if len(e_parts) == 2 else None,
                        end_year                = int(e_parts[0]) if len(e_parts) >= 1 and e_parts[0] else None,
                    ))
    ...
    def fetch_application(jd_id: str, candidate_id: str, db: Session):
        ...
        try:
            ...
            data = {
                ...
                "education": list({
                    (edu.school, edu.degree, edu.discipline, edu.start_year, edu.end_year): {
                ...
                "experience": list({
                    (exp.company, exp.job_title, exp.start_year, exp.end_year): {
                        "total_experience": exp.total_experience,
                        "relevent_experience": exp.relevent_experience,
                        "current_company": exp.company,
                        "current_job_title": exp.job_title,
                        "notice_period": exp.notice_period,
                        "current_ctc": exp.current_ctc,
                        "expected_ctc": exp.expected_ctc,
    ...
    @staticmethod
    def fetch_candidate_profile(candidate_id: str, db: Session):
        """
        Fetches the complete profile for a candidate, including:
        - Account (identity)
        - Profile Details (gender, location, etc.)
        - Education history
        - Work experience (all records)
        - Certificates
        """
        try:
            candidate = db.query(CandidateAccount).options(
            ...
            data = {
                ...
                "experience": list({
                    (exp.company, exp.job_title, exp.start_year, exp.end_year): {
                        ...
                        "is_current_designation": exp.is_current_designation,
                    } for exp in (candidate.experience or [])
                }.values()),
                "certificates": list({
                    (cert.name, cert.provider, cert.year): {
