...
@app.post("/admin/interview/sync-start", response_model=InterviewSession)
def sync_start_interview(payload: dict):
    """Bridge for 8001 to instantiate an AI Interview purely using dictionary metrics."""
    db.load()
    job_id = payload.get("job_id")
    application_id = payload.get("application_id")
    ai_intel = payload.get("ai_intelligence", {})
    
    jd = db.jds.get(job_id) if job_id else None
    if not jd or not jd.get("intelligence"):
        raise HTTPException(status_code=400, detail="Associated Job description is deleted or intelligence missing in 8003")
    ...
    s3_input = {
        "interview_requirements": jd_intel.get("interview_requirements", {}),
        "skill_intelligence": jd_intel.get("skill_intelligence", {}),
        "resume_claims": ai_intel.get("resume_claims", {}),
        "evidence_map": ai_intel.get("evidence_map", {}),
        "match_scores": ai_intel.get("match_scores", {}),
        "final_score": ai_intel.get("final_score", 0.0)
    }
    ...
    try:
        from interview_orchestration.graph import stage4_graph
        from interview_orchestration.nodes.initialize_interview import initialize_interview
        s4_state = initialize_interview({"candidate_id": str(application_id), "final_focus_areas": final_focus_areas})
        s4_res = stage4_graph.invoke(s4_state)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Interview initiation failed: {str(e)}")
    ...
    session = InterviewSession(
        ...
        candidate_id=0, # Artificial Override
        current_question=s4_res.get("current_question"),
        status="IN_PROGRESS" if s4_res.get("current_question") else "NOT_STARTED",
        created_at=datetime.now(), focus_areas=final_focus_areas,
        current_topic=s4_res.get("current_topic"),
...
@app.post("/admin/jd/sync", response_model=JobDescription, status_code=status.HTTP_201_CREATED)
async def sync_jd(payload: JDSyncPayload):
    """Sync JD text from port 8001 to run intelligence graph without file upload."""
...
@app.post("/admin/resume/sync", status_code=status.HTTP_201_CREATED)
async def admin_sync_resume(
    file: UploadFile = File(...),
    job_id: str = Form(...),
    candidate_id: str = Form(...)
):
    """Bridge for the Port 8001 application to offload inference without full local Auth."""
    target_jd = db.jds.get(job_id) or {}
    ...
    try:
        text = extract_text(file_content, file.filename)
        if text:
            print(f"Running Integrated Sync AI Intelligence for candidate {candidate_id}")
            state_input = {
        ...
        print(f"Resume Graph Integrated Sync Error: {e}")
        raise HTTPException(status_code=500, detail=f"Intelligence Extraction failed: {str(e)}")

    # We return the intelligence directly without writing to db.resumes locally.
    # The 8001 DB handles the truth state for integration deployments.
